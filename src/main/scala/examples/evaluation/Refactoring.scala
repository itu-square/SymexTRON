...
object Refactoring {
  ...
  val replaceDelegationWithInheritanceAst : Statement = stmtSeq(
    ...
    def executeRefactoring(name: String, initialMems: List[SMem], refactoring: Statement): Unit = {
      ...
      val tg = new WhiteBoxTestGenerator(FullClassModel.allDefsWithKeys, refactoring, beta=1, delta=1, kappa=2)
      val task: Task[Unit] = tg.generateTestsE(Set(initialMems : _*)).map(_.toString).to(io.stdOutLines).run
      println("-" * 20)
