...
object Subst {
  ...
  implicit class SubstExpr(e0: SetExpr[IsSymbolic.type]) extends Subst[SetExpr[IsSymbolic.type]] {
    ...
    override def subst(x: SetSymbol, e: SetExpr[IsSymbolic.type]): SetExpr[IsSymbolic.type] = e0 match {
      ...
      case Part(syms) => e match {
        case Part(parts) =>
          if (syms.contains(x)) Part(syms.filter(_ != x) ++ parts)
          else e0
        case e@SetSymbol(id) => Part(syms.map(y => if (x == y) e else y))
        case _ =>
          if (syms.contains(x)) Union(e, Part(syms.filter(_ != x)))
          else e0
      }
